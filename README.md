# 💻 Bash Scripting Blueprint: Zero to Hero

Built by Nkechi Anna Ahanonye — Cloud & DevOps Engineer | I turn manual, 3 AM-breaking deployments into 1-min automated pipelines with AWS + Ansible + Terraform.

For DevOps students who need relatable, hands-on examples — not textbook theory.

---

## Shell Scripting for DevOps

Master Bash scripting from your first terminal command to practical automation.

This free, self-paced blueprint takes you from **absolute beginner to confident Bash practitioner** through guided lessons, hands-on labs, checkpoints, quizzes, troubleshooting, and a final DevOps automation project.

You do not need to buy a course to follow this training.

## What You Will Learn

- Terminal and shell fundamentals
- Files and directories
- Bash scripts and executable permissions
- Variables and user input
- Conditionals
- Loops
- Functions
- Pipes and redirection
- Text processing with `grep`, `cut`, `sort`, `uniq`, `awk`, and `sed`
- Exit codes and error handling
- Safe Bash scripting
- Debugging
- DevOps automation
- A practical final project

## Learning Path

| Module | Topic | Level |
|---|---|---|
| 00 | Before You Start | Beginner |
| 01 | Terminal Basics | Beginner |
| 02 | Files & Directories | Beginner |
| 03 | Your First Bash Script | Beginner |
| 04 | Variables & Input | Beginner |
| 05 | Conditionals | Beginner |
| 06 | Loops | Beginner → Intermediate |
| 07 | Functions | Intermediate |
| 08 | Text Processing | Intermediate |
| 09 | Bash for DevOps | Intermediate |
| 10 | Safe Bash | Intermediate |
| 11 | Debugging | Intermediate |
| 12 | Final Project | Practical |

## How To Use This Repository

Start at **[00 - Before You Start](./00-before-you-start/README.md)**.

Do not rush through the modules.

For each lesson:

1. Read the concept.
2. Run the commands yourself.
3. Compare your result with the expected result.
4. Complete the exercise without looking at the solution.
5. Use the checkpoint to test your understanding.
6. Complete the quiz.
7. Move to the next module.

The goal is not to memorize commands.

The goal is to understand what you are doing and eventually automate tasks yourself.

## Prerequisites

You need:

- A computer
- Internet access for setup resources
- A terminal
- Basic ability to create and edit files

You do **not** need previous Bash programming experience.

Linux, macOS, WSL, and Git Bash are covered with platform notes where relevant.

## Cost

This blueprint is designed to be free.

Most lessons run locally and do not require AWS, Azure, or other paid cloud resources.

When a lab can create cloud costs, the lesson will clearly warn you and include cleanup guidance.

## Blueprint Series

| # | Blueprint | Category | Status |
|---|---|---|---|
| 1 | ansible-guide | Automation | ✅ Live |
| 2 | terraform-blueprint | IaC | ✅ Live |
| 3 | aws-blueprint | Cloud | ✅ Live |
| 4 | azure-blueprint | Cloud | ✅ Live |
| 5 | kubernetes-blueprint | Orchestration | ✅ Live |
| 6 | linux-blueprint | Fundamentals | ✅ Live |
| 7 | github-blueprint | Version Control | 🚧 Coming Soon |
| 8 | docker-blueprint | Containers | ✅ Live |
| 9 | **bash-scripting-blueprint** | Scripting | 🚧 Building |
| 10 | sdlc-blueprint | Methodology | 🚧 Coming Soon |
| 11 | cicd-blueprint | CI/CD | 🚧 Coming Soon |
| 12 | jenkins-blueprint | CI/CD | 🚧 Coming Soon |
| 13 | azure-devops-blueprint | DevOps Platform | 🚧 Coming Soon |
| 14 | prometheus-blueprint | Monitoring | 🚧 Coming Soon |
| 15 | grafana-blueprint | Visualization | 🚧 Coming Soon |
| 16 | openshift-blueprint | Enterprise K8s | 🚧 Coming Soon |
| 17 | cybersecurity-blueprint | DevSecOps | 🚧 Coming Soon |

## Connect

- **LinkedIn:** Nkechi Ahanonye
- **X:** @NAhanonye
- **Facebook:** Nkydigitech
- **Instagram:** @nahanonye
- **TikTok:** @nkechiahanonye
- **Portfolio:** nkydigitech.github.io/nky-portfolio

---

Built with ❤️ for the DevOps community.

Especially for African engineers who deserve accessible, relatable learning resources.
